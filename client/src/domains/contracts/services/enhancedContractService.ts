...
export const enhancedContractService = {
  ...
  async createContractTemplate(
    template: Omit<ContractTemplate, "id" | "is_active">
  ): Promise<string> {
    const { data, error } = await supabase
      .from("contract_templates")
      .insert({
  ...
  async updateContractStage(
    ...
    if (notes) {
      await this.addContractComment(contractId, notes, userId, false);
    }
  },
  ...
  async getContractVersions(contractId: string): Promise<ContractVersion[]> {
    ...
    const userIds = [
      ...new Set(
        (data || []).map((version) => version.created_by).filter(Boolean)
      ),
    ];
    let profiles: any[] = [];
  ...
  async createContractReminder(
    ...
    const { error } = await supabase.from("contract_reminders").insert({
      contract_id: contractId,
  ...
  async getContractAnalytics(teamId?: string): Promise<ContractAnalytics> {
    ...
    const signedContracts = contractsArray.filter(
      (c) => c.deal_stage === "signed"
    );
    const success_rate =
      total_contracts > 0
        ? (signedContracts.length / total_contracts) * 100
        : 0;
  ...
  async generateContractFromTemplate(
    ...
    const replacements = {
      teamName:
        contractData.team_name || pitchData?.teams?.team_name || "Team Name",
      playerName:
        contractData.player_name ||
        pitchData?.players?.full_name ||
        "Player Name",
      transferType:
        contractData.transfer_type || pitchData?.transfer_type || "Permanent",
      transferFee: contractData.transfer_fee || pitchData?.asking_price || "0",
      currency: contractData.currency || pitchData?.currency || "USD",
      annualSalary:
        contractData.annual_salary || pitchData?.player_salary || "0",
      signOnBonus:
        contractData.sign_on_bonus || pitchData?.sign_on_bonus || "0",
      performanceBonus:
        contractData.performance_bonus || pitchData?.performance_bonus || "0",
      ...
      clubRepresentative:
        contractData.club_representative || "Club Representative",
