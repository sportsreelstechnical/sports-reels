...
export class BulkPlayerUploadService {
  ...
  constructor(teamId: string, sportType: SportType, userId: string) {
    this.teamId = teamId;
  ...
  private getValidPositions(): string[] {
    const sportPositions: { [key in SportType]: string[] } = {
      ...
      cricket: ["Batsman", "Bowler", "Wicket-Keeper", "All-Rounder", "Fielder"],
  ...
  async uploadPlayers(
    ...
    const activityService = new PlayerActivityService(this.teamId, this.userId);
