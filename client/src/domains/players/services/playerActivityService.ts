...
export class PlayerActivityService {
    ...
    try {
      ...
      const { data: insertData, error } = await supabase
        ...
        .insert({
          ...
          performed_by: this.userId,
          player_name:
            data.new_values?.full_name ||
            data.old_values?.full_name ||
            "Unknown Player",
          action: data.action,
   ...
   */
  static generateActivityDetails(
    action: string,
    oldPlayer?: Partial<DatabasePlayer>,
    newPlayer?: Partial<DatabasePlayer>,
    changedFields?: string[]
      ...
      case "updated":
        ...
        const changeDescriptions = meaningfulChanges.map((field) => {
          switch (field) {
            ...
            case "leagues_participated":
              return "leagues participated";
            case "titles_seasons":
              return "titles and seasons";
