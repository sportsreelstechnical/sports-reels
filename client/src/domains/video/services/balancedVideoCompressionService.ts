...
import { RobustConstraintHandlingService } from "@/shared/utils/robustConstraintHandlingService";
...
export class BalancedVideoCompressionService {
  async compressVideo(
    ...
    console.log(
      `🔊 Audio preservation: ${options.preserveAudio ? "Yes" : "No"}`,
    );
    console.log(
      `🎬 Smooth playback: ${options.maintainSmoothPlayback ? "Yes" : "No"}`,
    );
    ...
    try {
      ...
      throw new Error(
        "Balanced compression failed. Please try a different approach.",
      );
    }
    ...
    console.log(
      `⚖️ BALANCED compression completed in ${processingTime.toFixed(0)}ms`,
    );
    console.log(
      `📉 Size reduction: ${((1 - result.compressionRatio) * 100).toFixed(1)}%`,
    );
      ...
      video.onloadedmetadata = async () => {
        ...
        if (options.preserveAudio) {
          try {
            ...
            console.warn(
              "⚠️ Failed to add audio track, continuing without audio:",
              error,
            );
          }
        }
        ...
        try {
          ...
          console.warn(
            "Failed to create MediaRecorder with high bitrate, trying fallback:",
            error,
          );
          mediaRecorder = new MediaRecorder(stream, {
            mimeType: "video/webm",
  ...
  private async balancedCompress(
    file: File,
    options: BalancedOptions,
  ): Promise<BalancedResult> {
      ...
      video.onloadedmetadata = async () => {
        ...
        try {
          if (fileSizeMB > 50) {
            ...
            console.log(
              `📁 Large file (${fileSizeMB.toFixed(1)}MB) - using no bitrate constraints for compatibility`,
            );
            mediaRecorder =
              RobustConstraintHandlingService.createMediaRecorderWithFallbacks(
                stream,
                mimeType,
              );
          } else {
            // Small files: try with optimized bitrates
            const optimalBitrates =
              RobustConstraintHandlingService.getOptimalBitrates(
          ...
          console.error(
            "Failed to create MediaRecorder with all fallbacks:",
            error,
          );
          reject(
            new Error(
              "Failed to create MediaRecorder for balanced compression",
            ),
          );
          return;
        }
          ...
          try {
            ...
            simpleMediaRecorder.onstop = async () => {
              try {
                ...
                reject(
                  new Error(`Balanced compression recovery failed: ${error}`),
                );
              }
            };
            ...
            const simpleProcessFrame = () => {
              if (
                video.ended ||
                simpleFrameCount >= Math.floor(video.duration * 15)
              ) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      video.onloadedmetadata = async () => {
        ...
        try {
          ...
          reject(
            new Error("Failed to create MediaRecorder for speed compression"),
          );
          return;
        }
          ...
          try {
            ...
            simpleMediaRecorder.onstop = async () => {
              try {
                ...
                reject(
                  new Error(`Speed compression recovery failed: ${error}`),
                );
              }
            };
  ...
  private async addAudioTrackToStream(
    stream: MediaStream,
    file: File,
  ): Promise<void> {
    try {
...
export const balancedVideoCompressionService =
  new BalancedVideoCompressionService();
