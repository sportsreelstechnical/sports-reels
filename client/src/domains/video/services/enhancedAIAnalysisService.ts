...
export class EnhancedAIAnalysisService {
  async analyzeVideo(
    ...
    try {
      ...
      if (videoError || !videoData) {
        ...
        throw new Error(
          "Video not found or you do not have permission to analyze it"
        );
      }
  ...
  private generateEventDescription(videoType: string, index: number): string {
    ...
    return (
      descriptions[videoType as keyof typeof descriptions]?.[index] ||
      "Significant event requiring detailed analysis"
    );
  }
  ...
  private generateRecommendations(
    ...
    return (
      recommendations[videoType as keyof typeof recommendations] || [
  ...
  private getEventDescription(type: string, videoType: string): string {
    if (videoType === "match") {
      const descriptions = {
        ...
        tactical:
          "Formation or strategy adjustment to counter opponent tactics",
        key_moment:
          "Decisive moment that significantly impacted match progression",
      };
        ...
        keyMoments: present
          ? [
              ...
              },
              {
                ...
                impact:
                  "Showed excellent positional awareness and decision-making",
              },
            ]
          : [],
  ...
  private detectMissingPlayers(
    ...
    metadata.playerTags.forEach((playerId: string) => {
      if (Math.random() < 0.3) {
        // 30% chance player is missing
        missing.push({
          playerId,
          playerName: `Player ${playerId}`,
          suggestion:
            "Check other recent videos for this player's performance analysis",
        });
      }
    });
  ...
  private async saveEnhancedAnalysis(
    ...
    try {
      ...
      const analysisData = {
        ...
        tagged_player_present: Object.values(result.taggedPlayerAnalysis).some(
          (p) => p.present
        ),
        game_context: {
