...
import {
  balancedVideoCompressionService,
  type BalancedOptions,
  type BalancedResult,
} from "./balancedVideoCompressionService";
...
export class FormatPreservingCompressionService {
  async compressVideo(
    ...
    console.log(
      `🎬 Format-preserving compression for ${fileSizeMB.toFixed(2)}MB ${originalFormat} file`,
    );
    ...
    if (options.preserveFormat) {
      try {
        ...
        console.warn(
          `Failed to preserve ${originalFormat} format, falling back:`,
          error,
        );
      }
    }
  ...
  private async compressAsMP4(
    file: File,
    options: FormatPreservingOptions,
  ): Promise<Omit<
    FormatPreservingResult,
    "originalFormat" | "compressedFormat" | "formatPreserved"
  > | null> {
    ...
    return new Promise((resolve, reject) => {
      ...
      video.onloadedmetadata = async () => {
        const settings = this.getMP4OptimizedSettings(
          file.size / (1024 * 1024),
          options.mode,
        );
        ...
        try {
          ...
          console.log(
            `✅ Created MP4 MediaRecorder with: ${supportedMimeType}`,
          );
        } catch (error) {
  ...
  > | null> {
    ...
    return new Promise((resolve, reject) => {
      ...
      video.onloadedmetadata = async () => {
        const settings = this.getWebMOptimizedSettings(
          file.size / (1024 * 1024),
          options.mode,
        );
        ...
        try {
          ...
          console.log(
            `✅ Created WebM MediaRecorder with: ${supportedMimeType}`,
          );
        } catch (error) {
  ...
  private async compressWithBestFormat(
    file: File,
    options: FormatPreservingOptions,
  ): Promise<
    Omit<
  ...
  > {
    ...
    return new Promise((resolve, reject) => {
        ...
        try {
          ...
          try {
            ...
            try {
              ...
              reject(
                new Error("All simple fallback MediaRecorder creation failed"),
              );
              return;
            }
          }
      ...
      video.onerror = () =>
        reject(
          new Error("Failed to load video for simple fallback compression"),
        );
      video.src = URL.createObjectURL(file);
    });
  }
  ...
  private async addSimpleAudioTrack(
    stream: MediaStream,
    file: File,
  ): Promise<void> {
    try {
  ...
  private async addMP4AudioTrackToStream(
    stream: MediaStream,
    file: File,
  ): Promise<void> {
    try {
      ...
      if (audioContext && audioContext.state === "running") {
        ...
        console.warn(
          "⚠️ Audio context not running, skipping audio track addition",
        );
      }
    } catch (error) {
  ...
  private async addWebMAudioTrackToStream(
    stream: MediaStream,
    file: File,
  ): Promise<void> {
    try {
...
export const formatPreservingCompressionService =
  new FormatPreservingCompressionService();
