...
export class ServerSideCompressionService {
  ...
  async compressVideo(
    ...
    try {
      ...
      const compressionJobId = await this.triggerCompression(
        originalKey,
        options,
      );
      ...
      const compressedKey = await this.waitForCompression(
        compressionJobId,
        options,
      );
      ...
      console.log(
        "🎉 SERVER-SIDE compression completed in",
        processingTime + "ms",
      );
      console.log(
        "📉 Size reduction:",
        ((1 - compressedInfo.size / file.size) * 100).toFixed(1) + "%",
      );
  ...
  private async uploadOriginalToR2(
    file: File,
    options: ServerCompressionOptions,
  ): Promise<string> {
  ...
  private async triggerCompression(
    originalKey: string,
    options: ServerCompressionOptions,
  ): Promise<string> {
    const compressionSettings = {
      ...
      targetBitrate:
        ...
        this.getOptimalBitrate(options.compressionQuality),
  ...
  private async waitForCompression(
    jobId: string,
    options: ServerCompressionOptions,
  ): Promise<string> {
    ...
    while (true) {
      try {
        const response = await fetch(
          `${this.API_BASE_URL}/compression-status/${jobId}`,
          {
            method: "GET",
            headers: {
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to check compression status: ${response.statusText}`,
          );
        }
        ...
        if (status.status === "completed") {
          ...
          if (progress >= lastProgress || progress === 100) {
            ...
            options.onProgress?.(progress);
            options.onStatusChange?.(
              `Phase 2 processing... ${Math.round(progress)}%`,
            );
          }
        }
        ...
        if (consecutiveErrors >= maxConsecutiveErrors) {
          throw new Error(
            `Compression failed after ${maxConsecutiveErrors} consecutive polling errors: ${error instanceof Error ? error.message : "Unknown error"}`,
          );
        }

        // Wait longer on errors to avoid overwhelming the server
        await new Promise((resolve) =>
          setTimeout(resolve, pollInterval * consecutiveErrors),
        );
      }
    }
  }

  private async getCompressedFileInfo(
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to get compressed file info: ${response.statusText}`,
      );
    }
