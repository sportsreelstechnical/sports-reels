...
export const useTeamProfileCompletion = () => {
  ...
  const checkTeamCompletion = useCallback(async () => {
    ...
    try {
      ...
      const teamData = teamDataResult as TeamData;

      // Check required team fields
      const requiredTeamFields = ["team_name", "country", "sport_type"];

      const missingTeamFields = requiredTeamFields.filter(
        (field) => !teamData[field] || teamData[field] === "",
      );

      if (missingTeamFields.length > 0) {
        missingRequirements.push(
          `Complete team profile: ${missingTeamFields.join(", ")}`,
        );
      }
      ...
      const incompletePlayersCount = (players || []).filter((player) => {
        ...
        const safePlayer = player as Record<string, unknown>;
        return requiredFields.some(
          (field) =>
            !safePlayer[field] ||
            safePlayer[field] === "" ||
            safePlayer[field] === null,
        );
      }).length;

      if (incompletePlayersCount > 0) {
        missingRequirements.push(
          `Complete ${incompletePlayersCount} player profile(s)`,
        );
      }
      ...
      if (videoCount < 5) {
        missingRequirements.push(
          `Upload ${5 - videoCount} more videos (minimum 5 required)`,
        );
      }

      // Determine completion status
      const isTeamProfileComplete =
        missingTeamFields.length === 0 &&
