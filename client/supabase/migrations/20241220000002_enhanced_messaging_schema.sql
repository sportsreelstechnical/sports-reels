...
-- This migration implements all the missing features for the transfer timeline messaging system
...
-- Message Violations Table for Flagged Content
...
CREATE INDEX IF NOT EXISTS idx_enhanced_messages_read_at ON enhanced_messages(read_at);
...
CREATE INDEX IF NOT EXISTS idx_enhanced_notifications_created_at ON enhanced_notifications(created_at DESC);
...
CREATE POLICY "Users can view messages they're involved in" ON enhanced_messages
    FOR SELECT USING (
        sender_id = (SELECT id FROM profiles WHERE user_id = auth.uid()) OR
        receiver_id = (SELECT id FROM profiles WHERE user_id = auth.uid())
    );

CREATE POLICY "Users can send messages" ON enhanced_messages
...
-- Trigger for creating contract notifications
...
GRANT ALL ON enhanced_notifications TO authenticated;
