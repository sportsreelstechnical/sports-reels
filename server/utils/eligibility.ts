...
function calculateClubMinutes(
  ...
  const metricsMinutes = metrics.reduce(
    (sum, m) => sum + (m.currentSeasonMinutes || 0),
    0
  );
  return Math.max(playerMinutes, metricsMinutes);
}

function calculateInternationalMinutes(
  player: Player,
  records: PlayerInternationalRecord[]
): number {
  const playerMinutes =
    (player.internationalMinutesCurrentSeason || 0) +
    (player.internationalMinutesLast12Months || 0);
...
function calculateVideoMinutes(
  videos: Video[],
  insights: VideoInsights[]
): number {
  const videoMinutesFromVideos = videos.reduce(
    (sum, v) => sum + (v.minutesPlayed || 0),
    0
  );
  const videoMinutesFromInsights = insights.reduce(
    (sum, i) => sum + (i.minutesPlayed || 0),
    0
  );
  return Math.max(videoMinutesFromVideos, videoMinutesFromInsights);
}

function calculateTotalCaps(records: PlayerInternationalRecord[]): {
  total: number;
  senior: number;
} {
  let total = 0;
...
export function calculateSchengenScore(data: ScoringData): VisaScoreResult {
  ...
  const intlMinutes = calculateInternationalMinutes(
    player,
    internationalRecords
  );
  ...
  const minutesScore = Math.min(
    40,
    (totalMinutes / MINIMUM_MINUTES_REQUIRED) * 40
  );
  ...
  const performanceScore = performanceMetrics
    ? Math.min(
        10,
        ((performanceMetrics.goals || 0) + (performanceMetrics.assists || 0)) *
          0.5
      )
    : 0;

  const totalScore = Math.round(
    minutesScore + internationalScore + leagueScore + performanceScore
  );
  ...
  if (totalMinutes < MINIMUM_MINUTES_REQUIRED) {
    recommendations.push(
      `Play ${
        MINIMUM_MINUTES_REQUIRED - totalMinutes
      } more minutes to reach minimum threshold`
    );
  }
  if (totalCaps < 5) {
    recommendations.push(
      `Earn ${5 - totalCaps} more international caps to strengthen application`
    );
  }
...
export function calculateO1Score(data: ScoringData): VisaScoreResult {
  ...
  const recognitionScore = hasRecognition
    ? 35
    : Math.min(35, (player.marketValue || 0) / 50000);

  const intlScore = Math.min(
    30,
    seniorCaps * 2 + (player.continentalGames || 0) * 3
  );

  const marketPercentile = Math.min(
    20,
    ((player.marketValue || 0) / 5000000) * 20
  );

  const aiAnalysisCount = videoInsights.filter((i) => i.aiAnalysis).length;
  const performanceScore = Math.min(
    15,
    aiAnalysisCount * 2 + (totalMinutes >= MINIMUM_MINUTES_REQUIRED ? 5 : 0)
  );

  const totalScore = Math.round(
    recognitionScore + intlScore + marketPercentile + performanceScore
  );
  const status = getStatus(totalScore);
  ...
  if (!hasRecognition) {
    recommendations.push(
      "Increase market value or obtain recognition/awards for extraordinary ability"
    );
  }
  if (seniorCaps < 10) {
    recommendations.push(
      `Earn ${10 - seniorCaps} more senior international caps`
    );
  }
  if (totalMinutes < MINIMUM_MINUTES_REQUIRED) {
    recommendations.push(
      `Record ${MINIMUM_MINUTES_REQUIRED - totalMinutes} more verified minutes`
    );
  }
...
export function calculateP1Score(data: ScoringData): VisaScoreResult {
  ...
  const videoPerformance =
    videos.length > 0 ? Math.min(20, videos.length * 4) : 0;
  ...
  const totalScore = Math.round(
    minutesScore + leagueScore + videoPerformance + validationScore
  );
  const status = getStatus(totalScore);
...
export function calculateUKGBEScore(data: ScoringData): VisaScoreResult {
  ...
  const gbePoints =
    nationalTeamPoints + clubLeaguePoints + continentalPoints + minutesPoints;
  ...
  if (gbePoints < 15) {
    ...
    recommendations.push(
      `Need ${pointsNeeded} more GBE points to qualify automatically`
    );

    if (seniorCaps < 5) {
      recommendations.push(
        `Earn ${5 - seniorCaps} senior international caps (+${
          5 - seniorCaps > 0 ? 5 : 0
        } points)`
      );
    }
    if (domesticMinutes < 600) {
      recommendations.push(
        `Play ${
          600 - domesticMinutes
        } more domestic league minutes (+4 points at 600 mins)`
      );
    }
    if (totalMinutes < MINIMUM_MINUTES_REQUIRED) {
      recommendations.push(
...
export function calculateESCScore(
  data: ScoringData,
  gbeResult: VisaScoreResult
): VisaScoreResult {
  const { player, metrics, videos, videoInsights, internationalRecords } = data;
  ...
  if (seniorCaps < 3) {
    recommendations.push(
      `Earn ${3 - seniorCaps} more senior caps to strengthen ESC case`
    );
  }
  if (totalMinutes < MINIMUM_MINUTES_REQUIRED) {
    recommendations.push(
  ...
  if (videoEvidence < 5) {
    recommendations.push(
      `Upload ${5 - videoEvidence} more video evidence clips`
    );
  }
...
export function calculateTransferEligibility(
  ...
  const clubMinutes = calculateClubMinutes(player, metrics);
  const internationalMinutes = calculateInternationalMinutes(
    player,
    internationalRecords
  );
  const videoMinutes = calculateVideoMinutes(videos, videoInsights);
  const totalMinutesVerified =
    clubMinutes + internationalMinutes + videoMinutes;

  const { total: totalCaps, senior: seniorCaps } =
    calculateTotalCaps(internationalRecords);
  ...
  const minutesNeeded = Math.max(
    0,
    MINIMUM_MINUTES_REQUIRED - totalMinutesVerified
  );
  ...
  if (minutesNeeded > 0) {
    recommendations.push(
      `Play ${minutesNeeded} more minutes to reach minimum ${MINIMUM_MINUTES_REQUIRED} minutes`
    );
  }

  if (capsNeeded > 0) {
    recommendations.push(
      `Earn ${capsNeeded} more senior international caps for UK GBE eligibility`
    );
  }
